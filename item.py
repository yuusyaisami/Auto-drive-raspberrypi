...
class RightBar:
    def __init__(self, x, y, Textbtn):
        ...
        self.rect = pg.Rect(x, y - diff,max_len,len(self.texts) * 24 + 5) 
        if f_y < 0:
            f_y = 0
        for i in range(len(self.texts)):
            self.Textbtns.append(TextButton(self.x,self.y + i * 24 + f_y - diff, len(Textbtn[i]) * 24 + 5,24,self.texts[i] ,18))
    def handle_event(self, event):
        if event.type == pg.MOUSEBUTTONDOWN:
            if self.rect.collidepoint(event.pos):
                pass
