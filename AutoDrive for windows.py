# プログラムの説明
# driverGUIはscene処理をmain関数のlocalとして宣言していたが
# それをやめて、globalとして宣言することにした
...
class DriverMapBox:
    ...
    def handle_event(self, event):
        if self.visible:
            ...
            #------------おわり  右クリックバー------------
            if event.type == pg.MOUSEBUTTONDOWN and not main_scene.menu_edit.cancelrightclicked:
                # ユーザーがDriverMapBoxをクリックしたとき
                if self.rect.collidepoint(event.pos):
                    if event.button == 1 and not main_scene.maps.exist_rightclick():
                        ...
                        if self.rightbar.rect.y + self.rightbar.size * len(self.rightbar.elements) > var.WINDOWNSIZE_Y:
                            ...
                            self.rightbar = gui.RowText(pg.Rect(x + 5,y - 5,140,32), ["road", "wall", "goal", "start", "cross traffic", "over traffic", "delete traffic"], var.FONT_SMALL,True,False,30,140,var.COLOR_INACTIVE,var.COLOR_ACTIVE,var.COLOR_ACTIVE,True,False )
                    elif driver.select_box_x == self.x and driver.select_box_y == self.y:
    ...
    def update(self):
        if self.visible:
            ...
            if self.box_cancel.clicked:
                main_scene.visible = True
                self.visible = False
                driver.screen = pg.display.set_mode((var.WINDOWNSIZE_X, var.WINDWONSIZE_Y))
    def draw(self, screen):
        if self.visible:
            for object in self.objects:
                object.draw(screen)
